...
,  const char*          argv[]
   ...
   {  mclx* tbl, *LONG, *nw, *buffer_nw, *buffer_long, *mxorder
   ;  mclv* Nssqs, *sums, *scratch_nw, *scratch_long, *todo
   ;  mcxIO* xfdata, *xforder
   ;  dim i, c, d
   ;  dim N, n_data, n_skip = 0, n_total = 0, n_skip_batch = 0, n_total_batch = 0
   ;  mcxIO* xfout = mcxIOnew("out.2T-away", "w")
   ;  mclv themclv
   ;  mclp themclp
   ...
   ;  if (argc < 4) mcxDie(1, me, "need <matrix> <pearson threshold> <final threshold> <order matrix>")
   ...
   ;  if (argc == 5)
      {  dim ii
      ;  xforder = mcxIOnew(argv[4], "r")
      ;  mxorder = mclxRead(xforder, EXIT_ON_FAIL)
      ;  for (ii=0;ii<N_COLS(mxorder);ii++)
         tbl->cols[ii].val = mxorder->cols[ii].n_ivps
      ;  mclxColumnsRealign(tbl, mclvValCmp)
   ;  }
   ...
   ;  final_THRESHOLD = acos(atof(argv[3]))
...
/*  if (0) mcxShuffle(order->ivps, i, sizeof themclp, (void*) &themclp) */
      ...
      ;  for (j=0;j<i;j++)
         ...
         ;  arc = acos(p)
                                    ...
                                    /* todo is the list of things to-do for new.
                                     * arc is the distance between j and k.
                                     * as they are in T-range, remove anything from todo
                                     * that is in the 2T away-range of k.
                                    */
         ;  if (arc <= THRESHOLD)
         ...
         ;  }
            ...
            {  n_class2++
            ;  if (n_pass_long < 1.0 * todo->n_ivps)     /* magic */
...
#define BUFFER_LONG_EDGES 0
   ...
   ;  fprintf(stderr, "skipped %d, nw has %d (%d per node), LONG has %d (%d per node)\n", (int) n_skip, (int) mclxNrofEntries(nw), (int) (mclxNrofEntries(nw) * 1.0 / N_COLS(nw)), (int) mclxNrofEntries(LONG), (int) (mclxNrofEntries(LONG) * 1.0 / N_COLS(LONG)))
   ;  mclxUnary(nw, fltxLQ, &final_THRESHOLD)
   ;  fprintf(stderr, "class 1,2,3  %d  %d  %d, final network %d\n", (int) n_class1,  (int) n_class2,  (int) n_class3, (int) mclxNrofEntries(nw))
